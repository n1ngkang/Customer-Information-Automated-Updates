...
## Project Structure
1. [categorize.js](./GAS_Automator_Code/categorize.js) - Data Categorization (Excel): Initial records are categorized and prepared in a local Excel environment to define the scope of work.
2. [auto_info_updates.py](./auto_info_updates.py) - Automated Processing (Python): The Python engine reads the Excel data, performs updates, and handles data transformation logic.
3. [check.js](./GAS_Automator_Code/check.js) - Synchronized Validation (Excel): Automates the synchronization of the Master Sheet by detecting manual check-box validations from the local Excel file to ensure cross-platform data consistency.
